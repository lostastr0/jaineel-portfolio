...
export default function About() {
  ...
  const label: React.CSSProperties = {
    ...
    textTransform: "uppercase", color: "rgba(200,200,216,0.45)",
    marginBottom: 14,
  };
  ...
  return (
    <>
      <style>{`
        .about-grid { display: grid; grid-template-columns: 1.3fr 1fr; gap: 10px; }
        .about-bio-card { grid-row: span 2; }
        .about-bottom-card { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
        @media (max-width: 768px) {
          .about-grid { grid-template-columns: 1fr !important; }
          .about-bio-card { grid-row: span 1 !important; }
          .about-bottom-card { grid-template-columns: 1fr !important; gap: 16px !important; }
        }
      `}</style>
      <section id="about" ref={ref} style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", position: "relative", overflow: "hidden", borderBottom: "1px solid rgba(255,255,255,0.06)", background: "radial-gradient(ellipse 60% 55% at 50% 40%, rgba(14,14,18,1) 0%, rgba(6,6,8,1) 60%, #020204 100%)" }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "80px clamp(20px, 5vw, 48px)", position: "relative", zIndex: 1, width: "100%" }}>
          ...
          <motion.div {...fadeUp(0)} style={{ marginBottom: 40, paddingBottom: 24, borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
              ...
              <span style={{ fontFamily: "var(--font-dm-mono)", fontSize: 10, letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(200,200,216,0.35)" }}>Brisbane, AU</span>
            </div>
          </motion.div>
          ...
          <div className="about-grid">
            ...
            {/* Bio card */}
            <motion.div {...fadeUp(0.1)} className="about-bio-card" style={{ ...card({ display: "flex", flexDirection: "column", gap: 0 }) }}>
              <div style={{ paddingBottom: 24 }}>
                <div style={label}>Bio</div>
                <p style={{ fontFamily: "var(--font-syne)", fontSize: "clamp(16px,1.8vw,24px)", fontWeight: 700, lineHeight: 1.35, letterSpacing: "-0.02em", color: "#efefef", marginBottom: 16 }}>
                  Cyber Security student.{" "}<span style={{ color: "#e0f2fe" }}>Self-taught</span> developer. Building at the intersection of{" "}<span style={{ color: "#e0f2fe" }}>security and code</span>.
                </p>
              ...
              <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 20, paddingBottom: 24 }}>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {TECH.map((item) => (
                    <span key={item} style={{ fontFamily: "var(--font-dm-mono)", fontSize: 10, color: "rgba(200,200,216,0.7)", letterSpacing: "0.08em", padding: "4px 10px", border: "1px solid rgba(255,255,255,0.07)", borderRadius: 2 }}>{item}</span>
                  ))}
                </div>
              </div>
              <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: 20, flex: 1 }}>
                ...
                {EDUCATION.map((item, i) => (
                  <div key={i} style={{ display: "flex", alignItems: "center", gap: 16, padding: "10px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <span style={{ fontSize: 12, color: item.active ? "#efefef" : item.muted ? "rgba(200,200,216,0.3)" : "rgba(200,200,216,0.6)", flex: 1 }}>{item.label}</span>
            ...
            {/* Goals + details */}
            <motion.div {...fadeUp(0.3)} className="about-bottom-card" style={card()}>
              <div>
              ...
              <div>
                ...
                {DETAILS.map(({ k, v, ice, green }) => (
                  <div key={k} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                    ...
                    <span style={{ fontFamily: "var(--font-dm-mono)", fontSize: 9, color: green ? "#4ade80" : ice ? "#e0f2fe" : "#efefef", display: "flex", alignItems: "center", gap: 5 }}>
